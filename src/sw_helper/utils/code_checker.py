                        ...
                        for dangerous_import in self.DANGEROUS_IMPORTS:
                            ...
                            if alias.name == module:
                                self._add_issue(
                                    ...
                                    code_snippet=(
                                        self._get_line_content(content, node.lineno)
                    ...
                    for subnode in ast.walk(node):
                        if isinstance(subnode, ast.Global):
                            self._add_issue(
                                ...
                                code_snippet=(
                                    self._get_line_content(content, subnode.lineno)
                                    if hasattr(subnode, "lineno") and subnode.lineno
                                    else None
                                ),
                            )
        except SyntaxError:
...
class CodeChecker:
    ...
    def __init__(self):
        self.checkers = [SecurityChecker(), PerformanceChecker(), MaintainabilityChecker()]
