...
@cli.command()
@click.argument("workflow_name", type=click.Choice(["stress_analysis", "modal_analysis", "topology_optimization"], case_sensitive=False))
@click.option("--cad-file", "-f", required=True, type=click.Path(exists=True), help="CAD file path (.FCStd, .step)")
@click.option("--parameters", "-p", multiple=True, help="Parameters in key=value format (e.g., thickness=10)")
@click.option("--material", "-M", default="steel", help="Material name (default: steel)")
@click.option("--load", "-l", multiple=True, help="Load definition (e.g., force=1000,direction=z)")
@click.option("--constraint", "-c", multiple=True, help="Constraint definition (e.g., fixed,bottom)")
@click.option("--mesh-size", "-s", type=float, default=2.0, help="Mesh element size (default: 2.0)")
@click.option("--output-dir", "-d", type=click.Path(), default="./", help="Output directory")
@click.option("--cad", type=click.Choice(["freecad", "mock"], case_sensitive=False), default="mock", help="CAD software type")
@click.option("--cae", type=click.Choice(["calculix", "mock"], case_sensitive=False), default="mock", help="CAE solver type")
@click.option("--mesher", type=click.Choice(["gmsh", "none"], case_sensitive=False), default="none", help="Mesh generator type")
@click.pass_context
def workflow(
    ctx,
    workflow_name,
    ...
    """
    Run CAD->CAE simulation workflow - execute complete analysis pipeline

    Workflow:
        1. Load CAD model
        2. Set parameters (optional)
        3. Export to STEP
        4. Generate mesh (optional)
        5. Setup simulation
        6. Run solver
        7. Extract results

    WORKFLOW_NAME: Analysis workflow type
        - stress_analysis: Static structural analysis
        - modal_analysis: Modal frequency analysis
        - topology_optimization: Topology optimization

    Examples:
        # Run stress analysis with mock connectors
        cae-cli workflow stress_analysis -f model.FCStd -M Q235 -l force=1000,direction=z -c fixed,bottom

        # Run modal analysis with custom mesh size
        cae-cli workflow modal_analysis -f bracket.FCStd --mesh-size 1.5 -d ./results

        # List available workflows
        cae-cli workflow --help
    """
    from pathlib import Path
    try:
        ...
        from src.integrations._base.workflow import WorkflowEngine
        from src.integrations._base.connectors import CADConnector, CAEConnector
    ...
    class FreeCADConnectorMock:
        ...
        def export_step(self, output_path: Path) -> bool:
            ...
            output_path.write_text("Mock STEP content")
            return True
    ...
    class MockCAE:
        ...
        def generate_mesh(self, geometry_file: Path, mesh_file: Path, element_size: float = 2.0) -> bool:
            ...
            mesh_file.write_text("Mock mesh content")
            return True
            
        def setup_simulation(self, mesh_file: Path, config: dict) -> Path:
            input_file = mesh_file.parent / "simulation.inp"
    ...
    # 尝试导入真实连接器，如果失败则使用模拟连接器
    try:
        from integrations.cad.freecad import FreeCADConnector as RealCADConnector
        from integrations.cae.calculix import CalculiXConnectorMock
        # 如果导入成功，使用真实连接器
        CADConnectorClass = RealCADConnector
        CAEConnectorClass = CalculiXConnectorMock
    except ImportError:
        # 使用模拟连接器
        CADConnectorClass = FreeCADConnectorMock
        CAEConnectorClass = MockCAE

    try:
        # 显示工作流信息
        console.print(
            Panel.fit(
                f"[bold cyan]CAE工作流执行[/bold cyan]\n"
                f"工作流: [green]{workflow_name}[/green]\n"
                f"CAD文件: [yellow]{cad_file}[/yellow]\n"
                f"材料: [blue]{material}[/blue]\n"
                f"网格尺寸: [magenta]{mesh_size}[/magenta]\n"
                f"输出目录: [dim]{output_dir}[/dim]",
                title="工作流配置",
                border_style="cyan",
            )
        )
            ...
            for c in constraint:
                ...
                for item in c.split(","):
                    if "=" in item:
                        ...
                        constraint_dict["type"] = c
                if constraint_dict:
                    constraint_list.append(constraint_dict)

        # 创建连接器
        cad_connector = FreeCADConnectorMock()
        cae_connector = MockCAE()
        ...
        config = {
            ...
            "material": material,
            "loads": load_list if load_list else [{"type": "force", "value": 1000, "direction": "z"}],
            "constraints": constraint_list if constraint_list else [{"type": "fixed", "location": "bottom"}],
        ...
        # 获取工作流步骤
        steps = engine.PREDEFINED_WORKFLOWS.get(workflow_name, [])
        for i, (module, action) in enumerate(steps, 1):
            console.print(f"  {i}. {module}.{action}")

        # 执行工作流
        with console.status(f"[bold green]正在执行 {workflow_name}..."):
            result = engine.run_workflow(workflow_name, config)
        ...
        if result["status"] == "completed":
            ...
            summary_table.add_column("步骤", style="cyan", justify="center")
            summary_table.add_column("状态", style="green", justify="center")
            summary_table.add_column("描述", style="white")

            for step in result["steps"]:
                status_icon = "✓" if step.status.value == "completed" else "✗"
                summary_table.add_row(
                    str(step.name.split(".")[-1]),
                    status_icon,
                    step.description
                )
            ...
            if engine.results:
                ...
                results_table.add_column("值", style="yellow")

                for key, value in engine.results.items():
                    results_table.add_row(str(key), str(value))

                console.print(results_table)

            # 输出文件信息
            console.print(f"\n[dim]输出目录: {result['output_dir']}[/dim]")
            ...
            console.print(f"[red]✗ 工作流执行失败[/red]")
            sys.exit(1)
...
def list_workflows():
    ...
    Display all predefined workflow templates that can be used with the workflow command
    """
    try:
        from integrations._base.workflow import WorkflowEngine
    except ImportError:
    ...
    console.print(Panel.fit(
        "[bold cyan]可用工作流模板[/bold cyan]\n\n"
        "使用 cae-cli workflow <workflow_name> 运行工作流",
        title="工作流列表",
        border_style="cyan",
    ))
    ...
    table.add_column("工作流名称", style="green")
    table.add_column("描述", style="white")
    table.add_column("步骤数", style="yellow", justify="center")
    
    workflows = WorkflowEngine.PREDEFINED_WORKFLOWS
    descriptions = {
        "stress_analysis": "静力结构分析 - 计算应力和变形",
        "modal_analysis": "模态频率分析 - 计算固有频率和振型",
        "topology_optimization": "拓扑优化 - 自动优化材料分布",
    }
    ...
    for name, steps in workflows.items():
        ...
        steps_table.add_column("序号", style="cyan", justify="center", width=4)
        steps_table.add_column("模块", style="green")
        steps_table.add_column("操作", style="yellow")
        
        for i, (module, action) in enumerate(steps, 1):
            steps_table.add_row(str(i), module, action)
